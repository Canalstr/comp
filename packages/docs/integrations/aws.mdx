...
description: 'Connect AWS to Passt for cloud security testing'
---

# AWS Integration

The AWS integration allows you to connect your AWS infrastructure to Passt for automated security testing using Security Hub, compliance monitoring, and risk assessment.
...
2. Admin access to your Passt workspace
...
3. **Choose a Region Code**
   - Decide which AWS region you want Passt to use (e.g., `us-east-1`, `us-west-2`)
   - Copy this region code — you’ll need it in the Passt UI

4. **Create an IAM User for Passt**
...
7. **Connect AWS to Passt**
   - Go to **Settings > Integrations** in your Passt dashboard
...
Passt uses a cross-account IAM role with read-only permissions and a required external ID to ensure secure, scoped access. This approach follows AWS best practices for secure third-party integrations.
