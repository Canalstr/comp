 ...
 * Uses API key authentication with organization context
 */
export class ApiClient {
   ...
   * Uses API key authentication + explicit org context
   */
  async call<T = unknown>(endpoint: string, options: ApiCallOptions = {}): Promise<ApiResponse<T>> {
    ...
    if (typeof window !== 'undefined') {
      const apiKey = env.COMP_API_KEY;
      if (apiKey) {
        headers['X-API-Key'] = apiKey;
        console.log('🎯 Using API key for authentication');
      } else {
        console.log('⚠️ No API key available for API authentication');
      }
    }
